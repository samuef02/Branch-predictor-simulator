...
import { XorIndexer } from "../indexing/XorIndexer";
import { assertPositiveInteger } from "../shared/BitString";

export interface GshareConfig {
  readonly type: "gshare";
...
export interface GshareState extends PredictorState {
  readonly type: "gshare";
...
export class GsharePredictor
  implements BranchPredictor<GshareConfig, GshareState>, MemoryMeasurable<GshareConfig>
{
  initialise(config: GshareConfig): GshareState {
  ...
  predict(execution: BranchExecution, state: GshareState): PredictionResult<GshareState> {
    const { index, calculation } = new XorIndexer().resolveIndex(execution, {
    ...
    return {
      ...
      trace: {
        ...
        compactExplanation: `gshare selects counter ${index}.`
      }
    };
  }
  ...
  memoryUsage(config: GshareConfig) {
    this.validateConfig(config);
  ...
  private validateConfig(config: GshareConfig): void {
